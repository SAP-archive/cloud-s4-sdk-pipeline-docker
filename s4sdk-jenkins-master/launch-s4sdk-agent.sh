...
SSH_USER=$1
SSH_HOST=$2
REMOTE_DIR=s4sdk_jenkins_agent

if [ -n "$3" ]; then
  DOCKER_IMAGE=$3
else
  DOCKER_IMAGE='s4sdk/jenkins-agent:latest'
fi
...
$SSH_COMMAND -tt $SSH_USER@$SSH_HOST "cd $REMOTE_DIR/cx-server; ./cx-server start_cache"</dev/null >/dev/null
